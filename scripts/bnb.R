...
calc_lower_bound <- function(dist_matrix, nodes = NULL) {
  ...
  if (isSymmetric(unname(dist_matrix))) {
    for (node in nodes) {
      ...
      max_min_edge <- ifelse(min_edge > max_min_edge, min_edge, max_min_edge)
    ...
    for (node in nodes) {
      ...
      max_min_edge <- ifelse(min_out_edge > max_min_edge, min_out_edge, max_min_edge)

      in_edges <- dist_matrix[,node]
      min_in_edge <- min(in_edges[in_edges > 0])
      max_min_edge <- ifelse(min_in_edge > max_min_edge, min_in_edge, max_min_edge)
